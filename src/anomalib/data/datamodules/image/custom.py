...
from anomalib.data.datasets.image.custom import CustomDataset
from anomalib.data.utils import Split, TestSplitMode, ValSplitMode

logger = logging.getLogger(__name__)


class Custom(AnomalibDataModule):
    """MVTec-based Custom Datamodule.

    Args:
        root (Path | str): Path to the root of the dataset.
            Defaults to ``"./datasets/anomalib"``.
        category (str): Category of the MVTec dataset (e.g. ``"mix"`` |
            ``"left"`` | ``"right"``). Defaults to ``"mix"``.
    ...
    def __init__(
        ...
        self.name = name if name else Path(root).parts[-1]
        self.root = Path(root)
        ...
        Example:
            ...
            Directory structure after download::
                ...
                └── MVTec/
                    ...
                    └── ...
        """
        if (self.root.resolve() / self.category).is_dir():
