...
IMG_EXTENSIONS = (".png", ".PNG", ".jpg", ".jpeg")
CATEGORIES = (
    "mix",
    "left",
    "right",
...
class CustomDataset(AnomalibDataset):
    """MVTec-based dataset class.

    Dataset class for loading and processing MVTec-based AD dataset images. Supports
    both classification and segmentation tasks.

    Args:
        root (Path | str): Path to root directory containing the dataset.
            Defaults to ``"./datasets/MVTec-based"``.
    ...
    Example:
        >>> from pathlib import Path
        >>> from anomalib.data.datasets import MVTec-basedDataset
        >>> dataset = MVTec-basedDataset(
        ...     root=Path("./datasets/MVTec-based"),
        ...     category="bottle",
        ...     split="train"
        ... )
    ...
    def __init__(
        self,
        root: Path | str = "./datasets/anomalib",
        name: str = None,
        category: str = "mix",
        ...
        self._name = name if name else Path(root).parts[-1]
...
def make_custom_dataset(
    ...
    Args:
        ...
        a  (str | Split | None, optional): Dataset split (train or test)
            Defaults to ``None``.
